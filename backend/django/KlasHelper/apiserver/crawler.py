...
def get_assignments(request, semester_code):
    try:
        user_set = models.UserTb.objects.get(klas_id=request.META.get("HTTP_ID"))
        class_dict = json.loads(user_set.lectures)
        class_list = str(class_dict[semester_code]).split(',')
    except models.UserTb.DoesNotExist:
        status = "NotUserError"
        return {'status': status, 'assignment': ""}
    except KeyError:
        status = "SemesterCodeError"
        return {'status': status, 'assignment': ""}
    class_list.pop()
    temp = np.char.array('https://klas.khu.ac.kr/course/viewCourseClassroom.do?COURSE_ID=' + semester_code + '_')
...
def get_semesters(request):
    semesters = ""
    try:
        user = models.UserTb.objects.get(klas_id=request.META.get("HTTP_ID"))
        status = "OK"
